...
function jsonpRequest(payload) {
  return new Promise((resolve, reject) => {
    const callbackName = `feriasCallback_${Date.now()}_${Math.random().toString(16).slice(2)}`;
    const url = new URL(sharedApiUrl);
    const script = document.createElement("script");
    const timeout = window.setTimeout(() => {
      cleanup();
      reject(new Error("Tempo limite ao sincronizar dados"));
    }, 20000);
    ...
    window[callbackName] = (data) => {
      cleanup();
      if (!data?.ok) {
        reject(new Error(data?.error || "Falha ao sincronizar dados"));
        return;
      }
      resolve(data);
    };
